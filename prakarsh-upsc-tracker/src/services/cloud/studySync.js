...
export async function pushStudySnapshot({ accessToken, userId, snapshot }) {
  ...
  const byTable = {
    subjects: snapshot.subjects || [], lectures: snapshot.lectures || [], revision_tasks: snapshot.tasks || [],
...
export async function pullStudySnapshot({ accessToken, userId }) {
  ...
  const [subjects, lectures, tasks, habits, habitLogs, settingsRows] = await Promise.all([
